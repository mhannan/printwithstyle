...
/* define variables */
$original_txt = $original_txt_br = $main = $couple = array();
...
for( $i = 0; $i < $total_rows; $i++ ) {
	...
	if ( $i == $bride_row || $i == $groom_row ) { /* bride groom row */ 
		...
		$y = $i == $bride_row ? ($y + $lh/8) : $y - ($lh/8); /* increase y axis by 5 in case of bride row */
		...
		$y = $y - 15; /* decreate the y axis by 15 after bride row and before groom row */
	...
	if ( $_align == 'right' ) { /* right align */
		...
		$x = ceil( ( $img_width - $line_Width ) / 2 ) + 5;
	} else { /* left align */
