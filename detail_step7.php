...
if ( isset( $_REQUEST['previous'] ) && $_REQUEST['previous'] ) {
	$_REQUEST = $_SESSION['wed']['step7'];
} else if ( isset( $_REQUEST['previous'] ) && !$_REQUEST['previous'] ) {
	/* update session step with updated request for previous step*/
	$_SESSION['wed']['step6'] = $_REQUEST;
}

if ( !isset($_SESSION['wed']['step7'] ) ) {
	$_SESSION['wed']['step7'] = array();
}
/* merge array current, previous and next */
$_REQUEST = array_merge( $_SESSION['wed']['step7'], $_SESSION['wed']['step6'], $_REQUEST );
?>
<div class="body_internal_wrapper">
	...
	<form name="frm_step7" id="frm_step7" method="post" action="add_to_cart.php">
			...
			<script type="text/javascript">
				...
				jQuery(document).ready(function($){
					$(".personalized_btn_back").live ('click', function() {
						$("#previous").val('1');
						$("#step").val('step6');
						$("#frm_step7").attr("action", '<?php echo siteURL;?>detail.php');
					});
				});
